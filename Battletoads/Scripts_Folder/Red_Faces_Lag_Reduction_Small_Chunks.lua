...
-- If a new object is written to the object table and RNG-2 doesn't have a correct value, then an earlier save state
-- is loaded. Random inputs are applied throughout this process, and the reloading of save states continues until a 
-- favorable RNG-2 value is reached.
...
-- 2500 trials are run using this process, and whenever an attempt generates a new lowest number of lag frames, the button presses that occured on attempt the attempt are written to a file named "resultsOfBestScriptFile.txt"
